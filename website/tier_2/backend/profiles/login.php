...
if (count($errors) > 0) {
	...
	if (count($errors) > 0) {
		...
		$check_mail = $db->count("edu_tier_2",["email" => $requests['data']['email']]);
		if ($check_mail < 1) {
		    $errors["email"][] = "Couldn’t find your Account.";
		} else {
		    
		    $data_users = $db->findOne("edu_tier_2",["email" => $requests['data']['email']]);
			...
			if (!password_verify($password, $data_users['password'])) {
				...
				$db->editId("edu_tier_2",["session_login" => $session_login],$id);
			}
		}
