...
var table_options = {
    url: "<?=set_route_to_link(["jsload","edu_tier","data_tier_4",$tier_3_id])?>", // link table
    ...
    columns: [
        ...
        { data: "count_ref_level", orderable: true },
    ...
    columnDefs: [        
        ...
        },
        {
            ...
            render: function(data, type, row, meta) {
                var id = row._id;
                return `<a href="<?=set_route_to_link(['public', 'edu_tier', 'tier_5'])?>/${id}" class="upload-btn"><i class="fas fa-folder-open"></i> ${data}</a>`;
            },
        },
        {
            targets: 5,
...
function add_database() {
    ...
    if (name == "" || email == "" || password == "" || tier_3_id == "") {
        $.toast({
            heading: 'Lỗi',
    ...
    $.ajax({
        url: '<?=set_route_to_link(["backend","edu_tier","add_tier_4"])?>',
        type: 'POST',
        data: { name: name, email : email, password : password, phone : phone, tier_3_id : tier_3_id, discount_percent : discount_percent, software_discount_percent : software_discount_percent, channel_revenue_discount_percent : channel_revenue_discount_percent },
        success: function(response) {
...
function update_database() {
    ...
    elm_name.html('Đang xử lý...');
    $.ajax({
        url: "<?=set_route_to_link(["backend","edu_tier","update_info_tier_4"])?>",
        type: "POST",
        data: {
            id: archive_id,
            name: name,
...
function update_password() {
    ...
    elm_name.html('Đang xử lý...');
    $.ajax({
        url: "<?=set_route_to_link(["backend","edu_tier","update_password_tier_4"])?>",
        type: "POST",
        data: {
            id: archive_id,
            password: password,
