...
var table_options = {
    ...
    columnDefs: [        
...

        {
            targets: 9,
            render: function(data, type, row, meta) {
                // Cấu hình hiển thị cho cột số 9 (index bắt đầu từ 0)
                // Cột này hiển thị số lượng học viên (count_tier_4) từ dữ liệu được trả về từ server
                // Dữ liệu được hiển thị trong một badge tròn với viền màu xanh
                return `<div class="video-count-display">
                    <span class="badge bg-light text-primary border border-2 border-primary rounded-pill px-3 py-2 d-inline-flex align-items-center">
                        <i class="fas fa-database me-2"></i> <!-- Icon cơ sở dữ liệu -->
                        <span class="fw-medium">${data}</span> <!-- Hiển thị số lượng học viên -->
                        <span class="ms-1 small text-primary opacity-75"></span> <!-- Phần tử trống để thêm thông tin bổ sung nếu cần -->
                    </span>
                </div>`;
            }
        },
