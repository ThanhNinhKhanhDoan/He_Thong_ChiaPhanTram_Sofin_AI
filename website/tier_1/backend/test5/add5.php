<?php
    ...
    $message = $_POST['message'];   // Lấy giá trị từ trường 'message'

    // Kiểm tra xem email đã tồn tại trong database chưa
    $existingUser = $db->findOne("tests5", ["email" => $email]);
    ...
    $datas = [
        ...
        "message" => $message      // Lưu nội dung người dùng
    ];
    ...
    $dataToInserts = $db->add("tests5", $datas);
    
    // Hiển thị thông báo thành công
    if($dataToInserts){
        echo json_encode(["status" => true, "message" => "Thêm dữ liệu thành công"]);
    }else{
        echo json_encode(["status" => false, "message" => "Thêm dữ liệu thất bại"]);
    }
?>
