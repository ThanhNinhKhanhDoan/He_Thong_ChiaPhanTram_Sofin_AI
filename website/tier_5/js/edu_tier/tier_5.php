...
var table_options = {
    url: "<?=set_route_to_link(["jsload","edu_tier","data_tier_5",$user_id])?>", // link table
    url_update_status : "<?=set_route_to_link(["backend","edu_tier","update_status_tier_5"])?>", // link xoá
...
function add_database() {
    ...
    $.ajax({
        url: '<?=set_route_to_link(["backend","edu_tier","add_tier_5"])?>',
        type: 'POST',
        data: { name: name, email : email, password : password, phone : phone, user_id : user_id },
        success: function(response) {
...
function update_database() {
    ...
    elm_name.html('Đang xử lý...');
    $.ajax({
        url: "<?=set_route_to_link(["backend","edu_tier","edit_tier_5"])?>",
        type: "POST",
        data: {
            id: archive_id,
            name: name,
