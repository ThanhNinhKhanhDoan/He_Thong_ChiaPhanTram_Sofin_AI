...
class RowsAjax {
	...
	_createInstance() {
		...
		this._datatable = jQuery("#datatableRowsAjax").DataTable({
			...
			info: false,
			ajax: "<?=set_route_to_link(["jsload","users_tier_1","index"])?>",
			order: [],
			...
			columnDefs: [
				{
					...
					render: function(data, type, row, meta) {
						return '<a href="<?=set_route_to_link(["public","users","edit"])?>/' + row.id + '">' + data + '</a>';
					}
				},
				{
					targets: 2,
...
<div class="mb-3 filled">
		<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="acorn-icons acorn-icons-money-bag mb-3 d-inline-block text-primary"><path d="M7.28571 18C4.91878 18 3 16.0812 3 13.7143L3 13C3 9.13401 6.13401 6 10 6V6C13.866 6 17 9.13401 17 13L17 13.7143C17 16.0812 15.0812 18 12.7143 18L7.28571 18Z"></path><path d="M12 7L13.3163 3.70923C13.5673 3.08168 13.6928 2.7679 13.592 2.56688C13.5496 2.48236 13.4842 2.41159 13.4032 2.36278C13.2106 2.24669 12.888 2.34749 12.2429 2.54909L11.5659 2.76064C10.9008 2.96851 10.5682 3.07244 10.2269 3.09833C10.0759 3.10979 9.92413 3.10979 9.77305 3.09833C9.43182 3.07244 9.09923 2.96851 8.43406 2.76064L7.7571 2.54909C7.11197 2.34749 6.78941 2.24669 6.59681 2.36278C6.51583 2.41159 6.45035 2.48236 6.40797 2.56688C6.30716 2.7679 6.43267 3.08168 6.68369 3.70923L8 7"></path><path d="M9.1237 13.8588C9.60927 14.0159 10.1583 14.0898 10.6275 13.8219C10.9373 13.6233 11.0171 13.3 10.9971 12.9097C10.9754 12.2907 10.4246 12.0506 10.0115 11.9859C9.68537 11.9097 9.26683 11.7734 9.0784 11.397C8.91534 10.9859 9.00412 10.3994 9.3683 10.173C9.7 9.96689 10.4083 9.919 10.8268 10.173"></path><path d="M10 9V10M10 14V15"></path></svg>
		<input class="form-control" placeholder="Poin" onclick="copyInputValue(this)" value="`+formatNumberWithDots(data)+`" style="width: 160px; margin-top: 10px;">
</div>
						`;
					}
				},
...
<div class="mb-3 filled">
		...
		<input class="form-control" placeholder="Telegram" onclick="copyInputValue(this)" value="`+data+`" style="width: 120px; margin-top: 10px;">
</div>
						`;
					}
				},
	...
	_addNewRowFromModal() {
		...
		$.ajax({
			url: "<?=set_route_to_link(["backend","users_tier_1","create"])?>",
			type: "POST",
			data: {
				name: name,
				email: email, 
	...
	_onDeleteClick() {
		...
		selectedRows.each(function() {
			...
			for (let i = 0; i < rowIndex.length; i++) {
				...
				$.ajax({
					url: "<?=set_route_to_link(["backend","users_tier_1","delete"])?>",
					type: "POST",
					data: {
						id: row_id,
						email: row_email
