...
var table_options = {
    url: "<?=set_route_to_link(["jsload","edu_tier","data_tier_3",$tier_2_id])?>", // link table
    url_update_status : "<?=set_route_to_link(["backend","edu_tier","update_status_tier_1"])?>", // link xoá
    ...
    columnDefs: [        
        ...
        },
        {
            ...
            render: function(data, type, row, meta) {
                var id = row._id;
                return `<a href="<?=set_route_to_link(['public', 'edu_tier', 'tier_4'])?>/${id}" class="upload-btn"><i class="fas fa-folder-open"></i> ${data}</a>`;
            },
        },
        {
            targets: 3,
...
function add_database() {
    ...
    if (name == "" || email == "" || password == "" || discount_percent == "" || software_discount_percent == "" || channel_revenue_discount_percent == "" || tier_2_id == "") {
        $.toast({
            heading: 'Lỗi',
    ...
    $.ajax({
        url: '<?=set_route_to_link(["backend","edu_tier","add_tier_3"])?>',
        type: 'POST',
        data: { name: name, email : email, password : password, discount_percent : discount_percent, software_discount_percent : software_discount_percent, channel_revenue_discount_percent : channel_revenue_discount_percent, representative_name : representative_name, phone : phone, tier_2_id : tier_2_id },
        success: function(response) {
            elm_name.html(`<i class="fas fa-save"></i> Tạo giảng viên`);
            if (response.stt) {
