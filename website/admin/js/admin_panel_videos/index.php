...
var table_options = {
    url: "<?=set_route_to_link(["jsload","admin_panel_videos","data_cates"])?>", // link table
    url_update_status : "<?=set_route_to_link(["backend","admin_panel_videos","update_status_cates"])?>", // link xoá
    ...
    elm_id_data_type: "data-type-load", // id của select data type
    ...
    columns: [
        ...
        { data: "count_storage", orderable: true },
    ...
    columnDefs: [
        ...
        },
        {
            ...
            render: function(data, type, row, meta) {
                var id = row._id;
                return `<a href="<?=set_route_to_link(['public', 'admin_panel_videos', 'archive'])?>/${id}" class="upload-btn"><i class="fas fa-folder-open"></i> Quản lý kho</a>`;
            },
        },
        {
            targets: 0,
        ...
        },
        {
            targets: 1,
            render: function(data, type, row, meta) {
                return `<div class="badge bg-light text-primary border border-2 border-primary rounded-pill px-4 py-2 d-inline-flex align-items-center shadow-sm">
    ...
    return $(`#${table_options.elm_table_id}`).DataTable({
        ...
        ajax: function(data, callback, settings) {            
            $.ajax({
                ...
                dataType: 'json',
                data: { 
                    status: $(`#${table_options.elm_id_status}`).val(), 
                    date_start: $('#date_start_time_int_hidden').val(), 
                    date_end: $('#date_end_time_int_hidden').val(),
                    data_type: $(`#${table_options.elm_id_data_type}`).val(),
                },
...
function add_database() {
    ...
    $.ajax({
        url: '<?=set_route_to_link(["backend","admin_panel_videos","add_cate"])?>',
        type: 'POST',
        data: { name: name, data_type: data_type },
        success: function(response) {
            elm_name.html(`<i class="fas fa-save"></i> Lưu chủ đề`);
            if (response.stt) {
...
function update_database() {
    ...
    elm_name.html('Đang xử lý...');
    $.ajax({
        url: "<?=set_route_to_link(["backend","admin_panel_videos","edit_cates"])?>",
        type: "POST",
        data: {
            id: archive_id,
            name: name
