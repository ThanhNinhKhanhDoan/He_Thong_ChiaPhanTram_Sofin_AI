...
var table_options = {
    url: "<?=set_route_to_link(["jsload","admin_panel_videos","data_archives",$dir_id])?>", // link table
    url_update_status : "<?=set_route_to_link(["backend","admin_panel_videos","update_status_archives"])?>", // link xoá
    ...
    columnDefs: [
        ...
        },
        {
            ...
            render: function(data, type, row, meta) {
                if (type === 'display') {
                    return `<div class="id-badge">
                        <span class="badge bg-light text-secondary rounded-pill px-2 py-1" 
                              title="${data}" 
                              data-bs-toggle="tooltip"
                              style="cursor: pointer; font-size: 0.85rem; transition: all 0.2s ease;">
                            <i class="fas fa-hashtag me-1 text-primary"></i>${meta.row + 1}
                        </span>
                    </div>`;
                }
                return data;
        ...
        },
        {
            targets: 2,
            render: function(data, type, row, meta) {
                return `<a href="javascript:void(0)" class="upload-btn" data-id="${row._id}"><i class="fas fa-upload"></i> <b>[${data}]</b> Upload 16:9</a>`;
            }
        },
        {
            targets: 3,
            render: function(data, type, row, meta) {
                return `<a href="javascript:void(0)" class="upload-btn portrait" data-id="${row._id}"><i class="fas fa-upload"></i> <b>[${data}]</b> Upload 9:16</a>`;
            }
        },
        {
            targets: 4,
            render: function(data, type, row, meta) {
                ...
                <div class="date-display">
                    ...
                    <span class="date-badge bg-light text-primary rounded px-2 py-1">
                        <i class="fas fa-calendar-alt me-1"></i>${day}/${month}/${year}
                    </span>
                </div>`;
            }
        },
        {
            ...
            render: function(data, type, row, meta) {
                return `
                <div class="creator-email-display" onclick="openUserDetailsModal('${row.u_email}')">
                    <div class="d-flex align-items-center">
                        <span class="badge bg-light text-primary border border-primary rounded-pill px-3 py-2">
                            <i class="fas fa-user-edit me-2"></i>
                            <span class="fw-medium text-truncate" style="max-width: 180px;" title="${row.u_email || 'N/A'}">${row.u_email || 'N/A'}</span>
                        </span>
                        <div class="ms-2 position-relative creator-info-tooltip" data-bs-toggle="tooltip" title="Người tạo">
...
// load status
table_load_status(table_options)
...
$(document).on('click', '.edit-archive-btn', function() {
    ...
    // Xóa tất cả từ khóa hiện tại trong form chỉnh sửa
    edit_keywordManager.clearKeywords();
    
    // Nếu có từ khóa, tách chuỗi từ khóa thành mảng và thêm từng từ khóa vào form
    if (row.keywords && row.keywords.trim() !== '') {
        const keywords = row.keywords.split(',');
        // Duyệt qua từng từ khóa và thêm vào trình quản lý từ khóa
        for (let i = 0; i < keywords.length; i++) {
            if (keywords[i].trim() !== '') {
                edit_keywordManager.addKeyword(keywords[i]);
            }
        }
    }
...
function table_create(table_options) {
    console.log($(`#${table_options.elm_id_status}`).val());
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response.stt) {
                ...
                // Xóa tất cả từ khóa hiện tại trong form chỉnh sửa
                add_keywordManager.clearKeywords();
                $.toast({
                    heading: 'Thành công',
                    text: response.data,
                    icon: 'success',
