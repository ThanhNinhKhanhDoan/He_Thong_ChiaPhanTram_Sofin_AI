...
if (!$dataUsers || !isset($_GET['status']) || !isset($_GET['date_start']) || !isset($_GET['date_end'])) {
    ...
    $where = ["status" => $status, "create_at" => ['$gte' => $date_start, '$lte' => $date_end]];
    // $where = [];
    $count_data = $db->count("edu_tier_1", $where);
    $datas =  $db->find("edu_tier_1", $where);
    $data_ajax = [];
    if ($count_data > 0) {
